...
{
    for (int i = 0; i < size; i++)  // Fix off-by-one error
    {
...
int fibonacci(int num, int *store)
{
    ...
    for (int i = 0; i <= num; i++)  // Use a loop to compute Fibonacci numbers up to `num`
    {
        if (i == 0) store[0] = 0;  // Base case
        else if (i == 1) store[1] = 1;  // Base case
        else store[i] = store[i - 1] + store[i - 2];  // Compute Fibonacci number iteratively
    }

    return store[num];  // Return the Fibonacci number at position `num`
}
